...
parser.add_argument('--re_file', type=str,  default="re_outputs_ch_di", help='输入re结果位置')
parser.add_argument('--type', type=str,  default="chemical-disease", help='边种类')
...
files = pd.read_csv('about/ch_di/test.tsv',sep='\t')
...
for i,call in enumerate(results[1]):
    ...
    if call > 0.8:
        entities = [files.iloc[i,2],files.iloc[i,3]]
        entities_all8.append(entities)
print(len(entities_all8))
...
a2b = DataFrame(entities_all8)
a2b.insert(1,":TYPE",'chemical_disease')
...
a2b.to_csv('about/'+date+'/cddata1.tsv', index=False,sep='\t')
